...
public class UserController {
    ...
    @GetMapping
    public ApiResponse<List<UserResponse>> list(@RequestHeader("X-Auth-Token") String token) {
    ...
    @PostMapping
    public ApiResponse<UserResponse> create(@RequestHeader("X-Auth-Token") String token, @Valid @RequestBody UserRequest request) {
        UserAccount user = authService.requireUser(token);
        authService.requireAnyRole(user, RoleType.ADMIN);
    ...
    @PutMapping("/{id}")
    public ApiResponse<UserResponse> update(@RequestHeader("X-Auth-Token") String token, @PathVariable Long id, @Valid @RequestBody UserRequest request) {
        UserAccount user = authService.requireUser(token);
        authService.requireAnyRole(user, RoleType.ADMIN);
        return ApiResponse.ok(authService.updateUser(id, request));
    }
